...
export async function fetchAirportSuggestions(ctx: SuggestionContext): Promise<AirportSuggestion[]> {
  // MOCK-EVERYTHING mode: skip the Anthropic call entirely and return a fixed
  // list (same suggestions for origin and destination). No API key or network.
  if (FEATURE_FLAGS.mockAll) return mock.mockAirportSuggestions();
